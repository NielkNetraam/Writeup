...
The model has an OOB estimate of error rate (in sample error): `r percent(as.numeric(md$err.rate[200,"OOB"]))`
...
cm
```
Based on the validation, the modal has an `r percent(as.numeric(cm$overall[1]))` accuracy, out sample error = `r percent(1-as.numeric(cm$overall[1]))`. The out of sample error is even better than expected (smaller than the in sample error)
